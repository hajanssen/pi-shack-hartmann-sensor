...
class Processor():
    # Set 
    def __init__(self,floorCap=60,minDist=20,NZernike=7,
                storeImages=False,NApatures=None,printZernikeEqasion= False,
                normZernike = True,
...
#            self.savedImages = [self.asNumpy(imgs) for imgs in self.loadedImages[1:]]
    ...
    def getCentroids(self,img):
        ...
        if self.scaleToUnitCircle:
            ...
            Centroid  = (Centroid[0] / (scaling / 2) - 1, Centroid[1]/ (scaling/2) - 1 )
        return Centroid  
    ...
    def getZernikeCoefficient(self,dX,dY):
        ...
        MeasurementVector = np.hstack((dX.T,dY.T))[np.newaxis].T # newaxis makes it operationally with Matrixes
    ...
    def generateZernikeFunctions(self,printZernikeEqasion=False):
        ...
        for ZBaseStr in NZBaseStr:
            (ZBase,Zdx,Zdy) = self.str2Equation(ZBaseStr,printZernikeEqasion=printZernikeEqasion)
